...
var leftVelocity, rightVelocity;
var drag;
...
function setup() {
    ...
    leftV0 = 0;
    rightV0 = 0;
    leftVelocity = createVector(0,0);
    rightVelocity = createVector(0,0);
...
var moveBall = () => {
    ...
    if(leftState === 'ONSLOPE') {
        ...
        leftSlopeMovement.x += (leftV0Sin * Math.cos(maxAlpha) - (gravity * Math.sin(maxAlpha) * Math.cos(maxAlpha) * leftGravityTime)) / 60;
        leftSlopeMovement.y += (leftV0Sin * Math.sin(maxAlpha) - (gravity * Math.sin(maxAlpha) * Math.sin(maxAlpha) * leftGravityTime)) / 60;

        leftBallMovement.x += leftSlopeMovement.x;
        leftBallMovement.y += leftSlopeMovement.y;
  
        console.log(leftSlopeMovement.y, leftMaxHeight, Math.pow(leftV0 * Math.sin(maxAlpha),2) / (2 * gravity));
        if(leftSlopeMovement.y > leftMaxHeight) {
            leftMaxHeight = leftSlopeMovement.y;
        }
    ...
    if(leftState === 'ONOWNSLOPE' && leftOldState === 'ONFLOOR') {
        ...
        leftOwnSlopeMovement.x += (leftV0Sin * Math.cos(maxAlpha) + (gravity * Math.sin(maxAlpha) * Math.cos(maxAlpha) * leftGravityTime)) / 60;
        leftOwnSlopeMovement.y += (-leftV0Sin * Math.sin(maxAlpha) - (gravity * Math.sin(maxAlpha) * Math.sin(maxAlpha) * leftGravityTime)) / 60;
...
var isOnFloor = () => {
    ...
    if(leftBallMovement.y + leftBallStartPoint.y <= 0 &&
        (leftBallStartPoint.x + leftBallMovement.x <= 600 - seesawArea - ballSlopeDiff && 
        leftBallMovement.x >= Math.cos(maxAlpha) * (seesawHalfLength + (6.5 / 10) * seesawHalfLength) - ballSlopeDiff)) {
    ...
    if(rightBallMovement.y + rightBallStartPoint.y < 0 &&
        (
            rightBallStartPoint.x + rightBallMovement.x >= -600 + seesawArea + ballSlopeDiff &&
            Math.abs(rightBallMovement.x) >= Math.cos(maxAlpha) * (seesawHalfLength + (6.5 / 10) * seesawHalfLength) - ballSlopeDiff
        )) {
